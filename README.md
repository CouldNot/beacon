...
- **No-password TUN** - a one-time install copies the helper scripts to `/usr/local/libexec/beacon` and adds a scoped `NOPASSWD` sudoers rule limited to those exact scripts. Subsequent up/down/switch run via `sudo -n`.
